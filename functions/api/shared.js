...
const FILE_PATH = "shared/inbox.md";
const BRANCH = "main";
...
// POST: add item to shared inbox (with author tag)
export async function onRequestPost(context) {
  try {
    ...
    await updateFile(context.env, newContent, sha, `shared: [${author}] ${item.trim().slice(0, 50)}`);
...
// DELETE: remove item from shared inbox
export async function onRequestDelete(context) {
  try {
    ...
    const newContent = lines.join("\n");
    await updateFile(context.env, newContent, sha, `shared: delete "${item.trim().slice(0, 50)}"`);
...
// PATCH: add context to a shared inbox item
export async function onRequestPatch(context) {
  try {
    ...
    const newContent = lines.join("\n");
    await updateFile(context.env, newContent, sha, `shared: context on "${parentItem.trim().slice(0, 40)}"`);
